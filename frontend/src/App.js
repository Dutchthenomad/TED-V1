...
const TreasuryPatternDashboard = () => {
  ...
  const PatternRow = ({ label, p }) => (
    <tr className="text-[11px]">
      ...
      <td className="px-2 py-1">{p?.current_peak ? Number(p.current_peak).toFixed(1) + 'x' : p?.ultra_short_prob ? ((p.ultra_short_prob * 100).toFixed(1) + '%') : p?.next_game_prob ? ((p.next_game_prob * 100).toFixed(1) + '%') : '—'}</td>
        ...
        <div className="col-span-3 bg-gray-800 border border-gray-700 rounded p-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {mlStatus?.learning_engine?.feature_weights && Object.entries(mlStatus.learning_engine.feature_weights).map(([k, v]) => (
              <div key={k} className="flex justify-between"><span>{k}</span><span className="text-gray-300">{Number(v).toFixed(2)}</span></div>
            ))}
          </div>
