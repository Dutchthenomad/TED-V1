...
class UltraShortClassifier:
    ...
    def _features_dict(self, f: GameSpecificFeatures) -> dict:
        ...
        return {
            ...
            "time_at_level": float(f.time_at_current_level),
            "accel": float(f.multiplier_acceleration),
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "weights_preview": {k: round(v, 4) for k, v in list(self.weights.items())[:10]},
        }
...
class GameAwareMLPatternEngine:
    ...
    def __init__(self, base_pattern_engine):
        ...
        self.ultra_short = UltraShortClassifier(learning_rate=0.05, threshold=0.6)
        self._last_features: Optional[GameSpecificFeatures] = None
        ...
        try:
            ...
            if p_ultra >= self.ultra_short.threshold and current_tick < 25:
                ...
                if gated != combined_prediction:
                    ...
                    gate_applied = True
            enhanced_result = {
                'predicted_tick': int(combined_prediction),
                ...
                'ml_enhancement': {
                    ...
                    'ultra_short_prob': float(p_ultra),
                    'ultra_short_gate_applied': gate_applied
                },
    ...
    def complete_game_analysis(self, completed_game):
        try:
            ...
            # Update learning engine weights if we have a prediction
            ...
            if self._last_features is not None:
                ...
                try:
                    ...
                    logger.error(f"UltraShortClassifier update failed: {e}")
        except Exception as e:
            self._error_count += 1
            self._last_error = str(e)
    ...
    def get_ml_status(self) -> Dict:
        ...
        return {
            ...
            'prediction_method': 'game_aware_ml',
            'learning_engine': perf,
            'performance': {
                'accuracy': perf.get('overall_accuracy', 0.0),
                'recent_accuracy': perf.get('recent_accuracy', 0.0),
                'total_predictions': perf.get('total_predictions', 0),
            },
            'ultra_short': self.ultra_short.get_status(),
